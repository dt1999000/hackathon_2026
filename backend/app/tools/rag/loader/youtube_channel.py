...
class YoutubeChannelLoader(BaseLoader):
    ...
    def load(self, source_content: SourceContent, **kwargs: Any) -> LoaderResult:
        ...
        Args:
            source_content: The source content containing the YouTube channel URL
        ...
        """
        channel_url = source_content.source

        if not any(
            pattern in channel_url
            for pattern in ["youtube.com/channel/", "youtube.com/c/", "youtube.com/@", "youtube.com/user/"]
        ):
        ...
        try:
            ...
            for i, video_url in enumerate(video_urls, 1):
                try:
                    ...
                    content_parts.append(f"\n{i}. Error loading video: {e}")

            content = "\n".join(content_parts)

        except Exception as e:
            raise ValueError(f"Unable to load YouTube channel {channel_url}: {e}") from e
