...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        # hanse_guide/.env (one level above ./backend/)
        env_file="../.env",
        env_ignore_empty=True,
        extra="ignore",
    )
    API_V1_STR: str = "/api/v1"
    SECRET_KEY: str = "hanseguide-dev-secret"
    ACCESS_TOKEN_EXPIRE_MINUTES: int = 60 * 24 * 8
    FRONTEND_HOST: str = "http://localhost:5174"
    FASTAPI_ENV: Literal["development"] | None = "development"

    PROJECT_NAME: str = "HanseGuide"
    ...
    FIRST_SUPERUSER_PASSWORD: str = "hanseguide-dev-password"
    OLLAMA_BASE_URL: str = "http://localhost:11434"
    OLLAMA_MODEL: str = "llama3.2"
    OLLAMA_TIMEOUT: float = 20.0
